(function(){
  ...
  if(toggle && links){
    ...
    links.querySelectorAll('a').forEach(function(a){
      a.addEventListener('click', function(){ setNavState(false); });
    });
  }
  ...
  // Everything below is non-hero UI (accordions, drawers, counters, decorative canvas...) that
  // doesn't need to be ready in the first paint. Running it all synchronously in one block was
  // showing up as several ~100-260ms long tasks right at page load, which can delay the browser
  // from responding to an early tap/scroll (hurts INP) for no visible benefit — none of these
  // features are needed before the user can actually reach them. Each init gets its OWN deferred
  // slot (not one batch) so the browser can paint/respond to input between them — batching them
  // into a single idle callback just traded many small blocking tasks for one bigger one.
  var deferNonCritical = window.requestIdleCallback || function(fn){ setTimeout(fn, 1); };
  [
